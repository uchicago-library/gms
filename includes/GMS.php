...
class GMS {
	...
	function __construct($clean) {
		$this->clean = $clean;
        $this->db = new PDO('sqlite:' . dirname(__FILE__) .  '/../metadata/gms');
	}
	...
	function search() {
		if ($this->clean['search'][0]) {
			...
			while ($s < count($this->clean['search'])) {
				if ($this->clean['search'][$s]) {
					if ($this->clean['searchfield'][$s] == 'searchall') {
						$whereq[] = sprintf("token = :where_token_%d", $s);
                        $params[':where_token_' . $s] = $this->tokenize($this->clean['search'][$s]);
					} else { 
						$whereq[] = sprintf("(searchtype = :where_searchtype_%d AND token = :where_token_%d)", $s, $s);
                        $params[':where_searchtype_' . $s] = $this->clean['searchfield'][$s];
                        $params[':where_token_' . $s] = $this->tokenize($this->clean['search'][$s]);
                    }
				}
				$s++;
			...
			while ($s < count($this->clean['search'])) {
				if ($this->clean['search'][$s]) {
					if ($this->clean['searchfield'][$s] == 'searchall') {
						$docq[] = sprintf("SELECT DISTINCT doc FROM search WHERE token = :doc_token_%d", $s); 
                        $params[':doc_token_' . $s] = $this->tokenize($this->clean['search'][$s]);
					} else {
						$docq[] = sprintf("SELECT DISTINCT doc FROM search WHERE (token= :doc_token_%d and searchtype= :doc_searchtype_%d)", $s, $s);
                        $params[':doc_searchtype_' . $s] = $this->clean['searchfield'][$s];
                        $params[':doc_token_' . $s] = $this->tokenize($this->clean['search'][$s]);
                    }
				}
				$s++;
